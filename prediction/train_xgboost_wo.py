...
def main():
    ...
    base_dir = "./log_prediction/xgboost_wo_imputation/input_{}_output_{}".format(str(config['in_len']), str(config['out_len']))
    ...
    with torch.no_grad():
        for test_step, (datas, data_ob_masks, data_gt_masks, labels, label_masks) in enumerate(test_dloader):
            b, t, c, n = datas.shape
            data_masks_list.append(data_ob_masks.cpu())
            datas[~data_ob_masks.bool()] = np.nan
            datas = datas - mean
        ...
        np.save(base_dir + "prediction_xgboost.npy", predictions)
