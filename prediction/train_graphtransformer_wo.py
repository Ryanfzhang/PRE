...
from model.graphtransformer_v3 import GraphTransformer
import numpy as np
...
def main(nth_test="all"):
    ...
    base_dir = "./log_prediction/graphtransformer_wo_imputation/input_{}_output_{}".format(str(config['in_len']), str(config['out_len']))
    ...
    model = GraphTransformer(config, mean, std)
    ...
    for epoch in train_process:
        ...
        for train_step, (datas, data_ob_masks, data_gt_masks, labels, label_masks) in enumerate(train_dloader):
            ...
            prediction = model(datas*data_ob_masks, mean, adj, node_type)
        ...
        if epoch % config['test_freq'] == 0 and epoch != 0:
            ...
            if chla_mae_mean < best_mae_chla:
                torch.save(model, base_dir + "best_dlinear_with_imputation_{}.pt".format(nth_test))
