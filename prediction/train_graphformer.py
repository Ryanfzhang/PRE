...
from utils import check_dir, masked_mae, masked_mse, masked_huber_loss, huber_loss
...
def main(nth_test="all"):
    ...
    for epoch in train_process:
        ...
        for train_step, (datas, data_ob_masks, data_gt_masks, labels, label_masks) in enumerate(train_dloader):
            ...
            # datas = model.normalize(datas)
            # labels = model.normalize(labels)
            prediction = model(datas, adj, node_type)
            loss = masked_huber_loss(prediction, labels, label_masks)
            # huber = huber_loss(prediction, labels)
            # masked_huber = ((huber * label_masks) ** 2).sum([0, 1]) / (label_masks.sum([0, 1]) + 1e-5)
            # huber_area1_mean = masked_huber[:, train_dataset.area1.astype(bool)].mean()
            # huber_area2_mean = masked_huber[:, train_dataset.area2.astype(bool)].mean()
            # huber_area3_mean = masked_huber[:, train_dataset.area3.astype(bool)].mean()
            # loss = (huber_area1_mean + 3*huber_area2_mean + huber_area3_mean)/5
        ...
        if epoch % config['test_freq'] == 0 and epoch != 0:
            ...
            chla_mae = (torch.abs(predictions - labels) * label_masks).sum([0, 1, 2]) / (label_masks.sum([0, 1, 2]) + 1e-5)
            chla_mse = (((predictions - labels) * label_masks) ** 2).sum([0, 1, 2]) / (label_masks.sum([0, 1, 2]) + 1e-5)
