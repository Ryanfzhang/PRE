...
def main(nth_test="all"):
    ...
    base_dir = "./log_prediction/graphtransformer_w_imputation/input_{}_output_{}".format(str(config['in_len']), str(config['out_len']))
    ...
    is_sea = torch.from_numpy(is_sea).long().to(device)
    ...
    model = GraphTransformer(config, is_sea, mean, std)
