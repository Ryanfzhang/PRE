...
def main(nth_test="all"):
    ...
    base_dir = "./log_prediction/tsmixer_wo_imputation/input_{}_output_{}".format(str(config['in_len']), str(config['out_len']))
    ...
    for epoch in train_process:
        ...
        for train_step, (datas, data_ob_masks, data_gt_masks, labels, label_masks) in enumerate(train_dloader):
            ...
            B, T, C, N = datas.shape
            datas = rearrange(datas*data_ob_masks, 'b t c n->(b n) t c')
