...
for test_step, (datas, data_ob_masks, data_gt_masks, labels, label_masks) in enumerate(test_dloader):
    ...
    tmp_data = rearrange(tmp_data, "b t c h w -> (b h w) t c")
    itp_data = torchcde.linear_interpolation_coeffs(tmp_data)
    imputed_data = rearrange(itp_data, "(b h w) t c -> b t c h w", b=datas.shape[0], h=datas.shape[3], w=datas.shape[4])
