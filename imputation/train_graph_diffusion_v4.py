...
from model.graphdiffusion_v4 import IAP_base

with open("./config/config.yaml", 'r') as f:
    config = yaml.safe_load(f)

base_dir = "./log/graph_diffusion_v4/"
...
# adj = np.eye(adj.shape[0])*adj.sum(1)-adj
# D = np.diag(np.sum(adj, axis=-1)**(-1/2))
# adj = np.eye(adj.shape[0]) - np.matmul(np.matmul(D,adj),D)
...
for epoch in train_process:
    ...
    for train_step, (datas, data_ob_masks, data_gt_masks, labels, label_masks) in enumerate(train_dloader):
        ...
        loss = model.trainstep(datas, data_ob_masks, adj, node_type, is_train=1)
    ...
    if epoch % config['test_freq'] == 0 and epoch != 0:
        ...
        for test_step, (datas, data_ob_masks, data_gt_masks, labels, label_masks) in enumerate(test_dloader):
            ...
            imputed_data = model.impute(datas, data_gt_masks, adj, node_type, config['num_samples'])
