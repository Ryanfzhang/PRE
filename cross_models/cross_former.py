...
class Crossformer(nn.Module):
    ...
    def forward(self, x_seq):
        batch_size , t, c, n =  x_seq.size()
        x_seq = torch.permute(x_seq, (0,3,1,2))
        x_seq = x_seq.reshape(batch_size*n, t, c)
        ...
        dec_in = repeat(self.dec_pos_embedding, 'b ts_d l d -> (repeat b) ts_d l d', repeat = batch_size*n)
        predict_y = self.decoder(dec_in, enc_out)


        prediction = base + predict_y[:, :self.out_len, :]
        prediction = prediction.reshape(batch_size, n, self.out_len, -1)
