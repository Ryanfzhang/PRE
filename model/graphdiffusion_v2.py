...
class SpatialTemporalEncoding(nn.Module):
    def __init__(self, config, low_bound, high_bound) -> None:
        ...
        self.spatial_encoding = GCN(self.config['hidden_channels'], self.config['hidden_channels'], 3)
...
class GATSingleHead(nn.Module):
    ...
    def forward(self,
        ...
        node_feats_ = self.linear(node_feats)
        f1 = torch.matmul(node_feats_, self.v0)
        f2 = torch.matmul(node_feats_, self.v1)
        f2T = rearrange(f2, 'b n c-> b c n')
        attn_logits = F.leaky_relu(adj_matrix * (f1 + f2T))
        attn_logits = attn_logits.to_sparse()
        attn_probs = torch.sparse.softmax(attn_logits/ self.temp, dim=1)
        attn_probs = attn_probs.to_dense()
        node_feats = torch.matmul(attn_probs, node_feats_) + node_feats

        return node_feats

class HGAT(nn.Module):
    def __init__(self, c_in, c_out, num_types, temp=1.):
        super().__init__()
        self.c_in = c_in
        self.c_out = c_out
        self.num_types = num_types
    ...
    def forward(self, node_rep, adj_matrix, node_type):
        ...
        node_type = node_type.unsqueeze(0).unsqueeze(-1).expand(B, -1, -1)

        node_rep_ = self.projection(node_rep)
        ks = torch.cat([self.k_linears[i](node_rep_) for i in range(self.num_types)], dim=-1)
        vs = torch.cat([self.v_linears[i](node_rep_) for i in range(self.num_types)], dim=-1)

        f1 = torch.gather(ks, 2, node_type)
        f2 = torch.gather(vs, 2, node_type)
        f2T = rearrange(f2, 'b n c-> b c n')

        attn_logits = F.leaky_relu(adj_matrix * (f1 + f2T), 0.2)
        attn_logits = attn_logits.to_sparse()
        attn_probs = torch.sparse.softmax(attn_logits/ self.temp, dim=1)
        attn_probs = attn_probs.to_dense()
        node_rep = torch.matmul(attn_probs, node_rep_) + node_rep
...
class GCN(nn.Module):
    ...
    def forward(self,
        ...
        position_embedding = torch.matmul(adj_matrix, position_embedding)
        position_embedding = torch.matmul(adj_matrix, position_embedding)
        # position_bias = torch.einsum('bnd,do->bno', position_embedding, self.bias_pool)
        position_weights = torch.einsum('nd, dio-> nio', position_embedding.mean(0), self.weights_pool)
        node_feats = torch.einsum('bni, nio->bno', node_feats, position_weights)
        # node_feats = self.linear(node_feats)+position_bias
        # node_feats = self.linear(node_feats) + self.linear_bias(position_embedding)
        return node_feats
