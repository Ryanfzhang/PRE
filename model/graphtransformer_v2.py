...
class GraphTransformer(nn.Module):
    ...
    def forward(self, x_enc, adj):
        ...
        D = enc_out.shape[-1]
        enc_out = rearrange(enc_out, 'b l d n -> (b l) n d', b=B, n=N)
        enc_out = self.spatial_encoder(enc_out, adj, position_embedding.transpose(0,1))
        enc_out = self.norm1(enc_out)
        enc_out = rearrange(enc_out, '(b l) n d -> (b n) l d', b=B, n=N)
        # enc_out, _ = self.temporal_encoder(enc_out, enc_out, enc_out)
