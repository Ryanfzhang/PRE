...
from .layers import (HorBlock, ChannelAggregationFFN, MultiOrderGatedAggregation, Attention,
...
class TCUSubBlock(nn.Module):
    def __init__(self, dim, mlp_ratio=1., drop=0.,drop_path=0., init_value=1e-2, act_layer=nn.GELU):
        super().__init__()
...
class TemporalAttention(nn.Module):
    ...
    def __init__(self, d_model, kernel_size=1, attn_shortcut=True):
        super().__init__()
...
class TemporalAttentionModule(nn.Module):
    ...
    def __init__(self, dim, kernel_size, dilation=1, reduction=1):
        super().__init__()
        d_k = 2 * dilation - 1
        d_p = (d_k - 1) // 2
        dd_k = kernel_size // dilation + ((kernel_size // dilation) % 2 - 1)
