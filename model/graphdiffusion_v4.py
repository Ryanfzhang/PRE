...
class SpatialTemporalEncoding(nn.Module):
    def __init__(self, config, low_bound, high_bound) -> None:
        ...
        self.mid_projection = Conv1d_with_init(2*config['hidden_channels'], 2*config['hidden_channels'], 1) 
        ...
        self.mean = torch.from_numpy(np.load('/home/mafzhang/data/PRE/8d/mean.npy')).to(self.device)
        self.std = torch.from_numpy(np.load('/home/mafzhang/data/PRE/8d/std.npy')).to(self.device)
    ...
    def forward(self, x, mask, adj, node_type, diffusion_step):
        ...
        out_temporal = rearrange(x, '(b k n) t c->(b t k) c n', b=B, n=N, k=K)

        #spatial encoding
        position_embedding = self.get_position_embedding_()
        x = rearrange(input, 'b t k c n-> (b t k) n c', b=B, k=K, t=T)
        x_in = x
        x = self.spatial_encoding(x, adj, position_embedding)
        x = x + x_in
        x = rearrange(x, '(b k t) n c-> (b t k) c n', b=B, k=K, t=T)
        out_spatial = self.gn(x)

        x = torch.concatenate([out_temporal, out_spatial], dim=1)
        ...
        # y = self.output_projection(x)
        # y = rearrange(y, '(b k n) c t -> b t k c n', b=B, k=K, n=N)
        # y = y.squeeze(3)
